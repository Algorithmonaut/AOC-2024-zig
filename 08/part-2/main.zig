...
fn set_antenna_antinodes(
    ...
    for (locations) |base_location| {
        for (locations) |other_location| {
            ...
            var antinode_pos_row = base_location[0];
            var antinode_pos_col = base_location[1];

            var i: u32 = 0;
            while (true) : (i += 1) {
                if (antinode_pos_col >= width or antinode_pos_col < 0) break;
                if (antinode_pos_row >= height or antinode_pos_row < 0) break;
                ...
                antinode_pos_row -= relative_row;
                antinode_pos_col -= relative_col;
            }
        }
    }
}
