...
pub fn main() !void {
    ...
    while (true) {
        ...
        sum += try process_line(line_buf.items, &compute);
...
fn process_line(line: []u8, compute: *bool) !isize {
    var sum: isize = 0;

    var line_parser = parser.Parser.init(line);

    var i: usize = 0;
    // NOTE: It sucks to set len - 6 because mul() may be at EOL, but I guess it
    // worked with my generated input hehehehe.
    blk: while (i < line_parser.input.len - 6) : (i += 1) {
        if (std.mem.eql(u8, line_parser.input[i .. i + 3 + 1], "do()"))
            compute.* = true;
        if (std.mem.eql(u8, line_parser.input[i .. i + 6 + 1], "don't()"))
            compute.* = false;
        ...
        if (std.mem.eql(u8, line_parser.input[i .. i + 3 + 1], "mul(")) {
            ...
            if (!is_valid_operation_input(operation_input) or compute.* == false) continue :blk;
